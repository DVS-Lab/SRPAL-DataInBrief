...
averaged_data_stan <- data_stan %>%
  group_by(sub, echo) %>%
  summarize(mean_stan = mean(mean_stan), mean_stan = mean(mean_stan)) %>%
  ungroup()

# Group data_stan by sub, task, and echo, calculate averages for mean_stan and vsmean_stan
averaged_data_nat <- data %>%
  group_by(sub, echo) %>%
  summarize(mean_nat = mean(mean_nat), vsmean_nat = mean(mean_nat)) %>%
  ungroup()
...
result_long <- result_long %>%
  mutate(echo = if_else(row_number() %in% 9:16, "Optimally Combined", echo))

# Add a column to data_stan named echo, where every row has "Optimally Combined"
averaged_data_stan$echo <- "Optimally Combined"
...
sem_data <- averaged_data_nat %>%
  ...
  summarize(mean = mean(mean_nat), sem = sd(mean_nat) / sqrt(n()))

# Calculate SEM for each group in data_stan
sem_data_stan <- averaged_data_stan %>%
  group_by(echo, flip_angle) %>%
  summarize(mean = mean(mean_stan), sem = sd(mean_stan) / sqrt(n()))
...
# Combine the SEM data frames
sem_data_combined <- bind_rows(sem_data, sem_data_stan)
sem_data_combined$echo <- factor(sem_data_combined$echo, levels = c("Optimally Combined", "1", "2", "3", "4", "5"))
...
standard_theme <- function() {
  theme_minimal() +
    theme(
      ...
      legend.title = element_text(size = 10),  # Remove legend title
...
echo_data_comb <- subset(averaged_data_stan, echo == "Optimally Combined")

# Perform t-test
t_test_result_comb <- t.test(mean_stan ~ flip_angle, data = echo_data_comb)
...
ggplot(sem_data_combined, aes(x = echo, y = mean, fill = flip_angle)) +
  ...
  labs(title = "Average Whole Brain tSNR",
       x = "Echo",
       y = "tSNR",
       fill = "Variable") +
