...
one_second_trials = 15;
three_second_trials = 10;
five_second_trials = 3;
eight_second_trials = 2;
...
ITI_list = [repmat(1,1,one_second_trials) repmat(3,1,three_second_trials) repmat(5,1,five_second_trials) repmat(8,1,eight_second_trials)];
...
ISI_list = [repmat(1,1,one_second_trials_ISI) repmat(2.5,1,twopointfive_second_trials_ISI) repmat(4,1,four_second_trials_ISI)] + .5; % Added .5 seconds to the list.
...
A = combnk(0.06:0.08:0.5,2); % 10 possible combinations we will use for proposers A = combnk(0.06:0.13:0.5,2)
B = fliplr(A); % To counterbalance.
Recipient_Options = [A;B]; % Now the options are counterbalanced
