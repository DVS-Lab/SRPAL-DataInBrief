...
if DEBUG:
    ...
    if gui.OK:
        ...
        specific_run=subjDlg.data[4]
        screen=subjDlg.data[5]
...
run_data = {
    ...
    'Description': 'rf1-aging Trust Game Task'
    }
...
#Loading screen
Loading_screen = visual.TextStim(win, text='... Loading partner responses, please wait ...', pos = (0,1), wrapWidth=30, height = 1.2)
...
def do_run(run, trials):
    ...
    for trial in trials:
        ...
        if image_label == 'stranger':
            image = os.path.join(imagepath, 'T_%s_%s.jpg') %(stranger_id,agerange)
        else:
