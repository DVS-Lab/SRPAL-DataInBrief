...
if not os.path.exists(subjdir):
    os.makedirs(subjdir)
log_file = os.path.join(subjdir,'sub-{}_Trust-Ratings.csv')
...
for trial in trials:
    image_label = stim_map[trial['Image']]
    condition_label = partner_map[trial['Partner']]
    ...
    markerStart='0',marker='triangle', textSize=.5, showValue=True, scale = '-5 = not at all, 0 = neutral,  5 = very',
    ...
    while scale.noResponse:
        ...
        pictureStim.draw()
        questionStim.draw()
        ratingQ = 'Please rate how %s you feel %s is.' % (question_label, condition_label)
