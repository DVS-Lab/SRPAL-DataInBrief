...
if DEBUG:
    ...
    subjDlg.addField('Role:', choices=['Investor', 'Investee'])
    ...
    if gui.OK:
        ...
        #        run = int(specific_run);
        #        print('Running Run %s') %specific_run
        #    else:
...
win = visual.Window([800,600], monitor="testMonitor", units="deg", fullscr=True, allowGUI=False, screen=1) #set screen to 1 for dual monitor
...
loading = visual.TextStim(win, text="Loading...", height=1.5)
...
outcome_loss = visual.TextStim(win,name='text',text='x',font='Wingdings',pos=(0, 2.0), height=2, wrapWidth=None, ori=0, colorSpace='rgb', opacity=1,depth=-1.0)
outcome_win = visual.TextStim(win,name='text',text='þ',font='Wingdings',pos=(0, 2.0), height=2, wrapWidth=None, ori=0, colorSpace='rgb', opacity=1,depth=-1.0)

outcome_map = {
    1: 'You have chosen to keep the money',
    2: {0:'{} kept money', 1:'{} shared money'},
    999: 'You have three seconds to choose'
    }

# instruction screen #
instruct_screen = visual.TextStim(win, text='Welcome to the Investment Game!\nYou have been assigned the role of investor.\n\nIn this game you will be interacting with a few different partners: your friend, a stranger, and the computer.\n\nEach round, you will begin with $8.\n\n You can choose to send a portion of that money to your partner by selecting one of the options on the screen.', pos = (0,1), wrapWidth=20, height = 1.2)
instruct_screen2 = visual.TextStim(win, text='Press with your pointer finger to send the amount on the lower left of the screen. Press with your middle finger to send the amount on the lower right of the screen.\n\n Remember, whatever you send means your partner receives 3 times that amount.\n\n If you send money you will see whether your partner chose to share it back evenly with you or kept it all for themself.', pos = (0,1), wrapWidth=20, height = 1.2)

#Loading screen
Loading_screen = visual.TextStim(win, text='... Loading practice responses, please wait ...', pos = (0,1), wrapWidth=30, height = 1.2)
...
'''
#read in stimuli
trial_data_1 = [r for r in csv.DictReader(open('params/sub-' + subj_id + '/sub-'
    + subj_id + '_run-01_design.csv','rU'))]
#trial_data_2  = [r for r in csv.DictReader(open('params/sub-' + subj_id + '/sub-'
#    + subj_id + '_run-02_design.csv','rU'))]
#trial_data_3  = [r for r in csv.DictReader(open('params/sub-' + subj_id + '/sub-'
#    + subj_id + '_run-03_design.csv','rU'))]
#trial_data_4  = [r for r in csv.DictReader(open('params/sub-' + subj_id + '/sub-'
#    + subj_id + '_run-04_design.csv','rU'))]
#trial_data_5  = [r for r in csv.DictReader(open('params/sub-' + subj_id + '/sub-'
...
def do_run(run, trials):
    ...
    for trial in trials:
        ...
        if image_label == 'Computer':
            image = os.path.join(imagepath, 'computer.png')
        else:
            image = os.path.join(imagepath, 'practice.png')
        pictureStim.setImage(image)
        #retain image proportions while scaling them to within maximum height
        ...
        if len(resp) > 0:
            if (int(trial['cLeft']) == 0 and resp_val == 1) or (int(trial['cRight']) == 0 and resp_val == 2):
                fixation.draw()
                win.flip()
        ...
        if len(resp) > 0:
            if (int(trial['cLeft']) == 0 and resp_val == 1) or (int(trial['cRight']) == 0 and resp_val == 2):
                ...
                if resp_val == 999:
                    outcome_stim.setColor('#FFFFFF')
                else:
                    if partner_resp == 1:
                        outcome_stim.setColor('lime')
                        outcome_win.setColor('lime')
                        outcome_img = outcome_win
                    if partner_resp == 0:
                        outcome_stim.setColor('darkred')
                        outcome_loss.setColor('darkred')
                        outcome_img = outcome_loss
                outcome_onset=globalClock.getTime()
                outcome_img.draw()
                outcome_stim.draw()
                win.flip()
