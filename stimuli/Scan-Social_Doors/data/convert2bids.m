...
maindir = fullfile('/ZPOOL/data/projects/rf1-sra');
warning off all
...
subs = load("/ZPOOL/data/projects/rf1-sra-data/code/sublist_all.txt");
    ...
    for t = 1:length(tasks)
        for n = 1:length(nums)    
            ...
            if isfile(inputname)
                ...
                %rt_numeric = str2double(T.rt);
                %rt_numeric(isnan(rt_numeric)) = NaN;
                
                %T.rt = cellfun(@(x) {x}, num2cell(rt_numeric), 'UniformOutput', false);
                %T.rt(cellfun(@isnan, T.rt)) = {'n/a'};

                T.rt(strcmpi(T.rt, "NaN")) = {'n/a'};
                %T.rt(cellfun(@(x) isnan(str2double(x)), T.rt)) = {'n/a'};
                %C = table2cell(T);
